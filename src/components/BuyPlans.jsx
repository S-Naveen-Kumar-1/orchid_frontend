...
export default function BuyPlansScreen({ navigation }) {
    ...
    const loadUser = async () => {
      try {
        ...
        if (parsed && (parsed.id || parsed._id)) {
          ...
          try {
            const res = await axios.get(`${BASE_URL}/users/${id}/purchases`);
            if (res?.data?.purchasedPlans) setPurchasedPlans(res.data.purchasedPlans || []);
            else if (res?.data) setPurchasedPlans(res.data.purchasedPlans || []);
          } catch (e) {
  ...
  const startPayment = async (plan) => {
    ...
    try {
      ...
      // fallback: try app activation endpoint
      try {
        ...
        console.warn('Activation error (frontend):', { status: activateErr?.response?.status, data: activateErr?.response?.data, message: activateErr?.message });
                  ...
                  {purchased ? (
                    <View style={{ marginTop: 8 }}>
                      ...
                      <Text style={{ color: '#fff' }}>
                        Sprays: {(purchased.spraysAllowed || 0) - (purchased.spraysUsed || 0)} remaining
                      </Text>
                    </View>
                  ) : (
